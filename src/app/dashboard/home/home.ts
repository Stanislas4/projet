...
private storage = inject(StorageService);
  ...
  addRecentItems(data: any[], page: string, icon: string): void {
    data.forEach(item => {
      this.recentItems.push({
        id: item.id,
        nom: item.nomComplet || item.nom_prenom,
        page: page,
        icon: icon,
        date: item.createdAt || item.date_debut_procedure || item.date_agression || new Date(),
        type: item.typeViolence || item.type_violence,
        typeClass: this.getTypeClass(item.typeViolence || item.type_violence)
      });
    });
  }
  ...
  updateViolenceDistribution(data: any[]): void {
    ...
    data.forEach(item => {
      const type = item.typeViolence || item.type_violence;
      if (type) {
  ...
  generateActivities(data: any[]): void {
    // Prendre les 5 derniers enregistrements
    const recent = data
    ...
    // Si pas assez de données, ajouter des activités factices
